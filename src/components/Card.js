...
function Card (props) {
    ...
    return (
        <animated.div className="d-inline-block l-card selectedCard" onMouseMove = {({clientX: x, clientY: y}) => set({ xys: calc(x, y)})} onMouseLeave={() => set({xys: [0,0,1]})} style={{ transform: moveProps.xys.interpolate(trans)}} onClick={(event) => props.click(props.item)}>
